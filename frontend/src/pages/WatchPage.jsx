...
const WatchPage = () => {
    ...
    const {id} =  useParams(); // id: "1023239"
    const [trailers, setTrailers] = useState([]);
    const [currentTrailerIndex, setCurrentTrailerIndex] = useState(0);
    const [loading , setLoading] = useState(true);
    const [content, setContent] = useState({}); // one object 
	...
	// handle for the movie (through id) which doesn't exist
	if (!content) {
		return (
        ...
        <div className="mx-auto container px-4 py-8 h-full ">
            ...
            {similarContent.length > 0 && (
				<div className='mt-12 max-w-5xl mx-auto relative'>
					<h2 className='text-3xl font-bold mb-4'>Similar Movies/Tv Show</h2>
						...
						if (content.poster_path === null) return null;
							return (
								<Link key={content.id} to={`/watch/${content.id}`} className='w-52 flex-none'>
									...
									/>
									<h3 className='mt-2 text-lg font-semibold'>{content.title || content.name}</h3>
								</Link>
							);
					})}
